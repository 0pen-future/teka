...
### Phase 9 follow-up (thực hiện)

- **L1** — Đã sửa: `WithinTx` khoá `class_curricula` (`LockCurriculum`, `FOR UPDATE`) rồi đọc lại và so sánh trong tx trước khi ghi; test `TestApplyRereadsCurriculumUnderLockAfterAConcurrentEdit` (integration) xác nhận `Apply` chờ một sửa đổi đang mở rồi trả 409 `CURRICULUM_DIFFERS` khi curriculum đã đổi dưới khoá.
- **L3** — Đã sửa: `resolveParentClass` duyệt tổ tiên bằng `WITH RECURSIVE` (giới hạn độ sâu, lọc theo `center_id`, scopelint sạch) và trả 422 khi gặp lại `selfID`; test `TestResolveParentClassRejectsALineageCycle` (unit) và luồng qua service trong integration đều xanh.
- **L4** — Đã sửa: `class-lineage-card.tsx` lọc bỏ log có `status_code >= 400` và mọi hành động `class_message.*` khỏi Lịch sử lớp qua `isClassEditLog`; actor hiện qua `actorLabel` export từ feature `audit`. Test mới: "hides failed requests and class_message actions from the change history" (`class-detail-program.test.tsx`).
- **L5** — Đã sửa: `library.DeleteTemplate` chạy trong tx khoá template `FOR UPDATE`; `classprogram.Apply` khoá template của phiên bản `FOR SHARE` trong cùng tx và kiểm `deleted_at IS NULL`. Test: `TestApplySerialisesWithTemplateDelete` (integration, package `library`).
- **L6** — Đã sửa: đổi chữ cảnh báo tab Buổi học thành "N buổi đã lên lịch (tính trên các buổi đã tạo, không tính buổi huỷ)"; cảnh báo chỉ hiện khi lệch không thể tự khớp lại (buổi thực đã vượt số buổi mẫu, hoặc lớp đã kết thúc). Test cập nhật + mới: "zips the countable sessions with the template lessons and flags the mismatch", "hides the mismatch warning while an open-ended class still trails the template" (`class-detail-program.test.tsx`).
- **Test còn thiếu — `LiveClassInCenter`**: đã có test riêng cho predicate này (đợt trước Phase 9, giữ nguyên).
- **Test còn thiếu — binding HTTP `PUT program`**: đã thêm `apps/api/internal/features/classprogram/handler_test.go` (file mới của package, trước đây chỉ có `integration_test.go`) với `TestApplyMissingTemplateVersionIdOverHTTP` (thiếu `template_version_id` → 422 `CodeValidation` kèm field), `TestApplyMalformedTemplateVersionIdOverHTTP` (uuid sai định dạng → 400 `CodeBadRequest`), và `TestApplyMalformedClassIdIs404OverHTTP` (id lớp sai định dạng trên path → 404). Cả ba đều xanh ngay lần chạy đầu — không phát hiện lỗi hành vi, chỉ lấp khoảng trống test.
- **Test còn thiếu — luồng lỗi web của card chương trình**: thêm "toasts the API's reason when applying fails for a reason other than a curriculum mismatch" và "toasts the API's reason when removing the program fails" (`class-detail-program.test.tsx`). Không phát hiện lỗi hành vi — `programErrorMessage` đã xử lý đúng từ trước.
- **Test còn thiếu — lỗi lessons ở tab Buổi học**: thêm "keeps the table usable, with a blank Buổi mẫu column, when the lesson list fails to load" (`class-detail-program.test.tsx`). Không phát hiện lỗi hành vi — cột Buổi mẫu đã tự về "—" khi `lessons` lỗi.
- **Hai câu trong phase spec**: đã sửa `phase-07-chi-tiet-lop-chuong-trinh-tab.md` — mục API mô tả `GET /program/lessons` trả toàn bộ tài liệu (web lọc `shared_with_students` mặc định, có switch "Hiện tất cả"); mục Verification đổi tên e2e spec thành `class-program.spec.ts` (tự tạo template/lớp qua API, dọn ở `afterEach`) thay vì `class-list.spec.ts` không tồn tại.
