...
// QRState marks a milestone in the QR handshake that a caller can surface to a
// user waiting on their phone.
type QRState string

const (
	// QRStateScanned means the QR image was read by the Zalo app; the account
	// holder still has to approve the login there.
	QRStateScanned QRState = "scanned"
	// QRStateConfirmed means the approval landed and the session is being
	// finalized.
	QRStateConfirmed QRState = "confirmed"
)

// QRCallbacks receives progress from LoginQR. Both hooks are optional — a zero
// QRCallbacks runs the login silently. They fire on LoginQR's own goroutine, so
// an implementation must not block.
//
// OnProgress is Teka's addition to the zcago port: upstream only hands back the
// QR image, which collapses "scanned" and "confirmed" into one opaque wait.
type QRCallbacks struct {
	OnQR       func(qrPNG []byte)
	OnProgress func(state QRState)
}

func (cb QRCallbacks) qr(png []byte) {
	if cb.OnQR != nil {
		cb.OnQR(png)
	}
}

func (cb QRCallbacks) progress(state QRState) {
...
// LoginQR performs interactive QR code login, reporting the QR image and the
// scan/confirm milestones through cb. On success it returns credentials for
// persistence and sets sess.DisplayName to the linked account's Zalo name.
func LoginQR(ctx context.Context, sess *Session, cb QRCallbacks) (*Credentials, error) {
	...
	scanned, err := qrWaitingScan(timeoutCtx, sess, ver, qrData.Code)
	if err != nil {
	...
	sess.DisplayName = userInfo.Info.Name
	if sess.DisplayName == "" && scanned != nil {
		sess.DisplayName = scanned.DisplayName
	}
	...
	// Credentials are validated by the caller via LoginWithCredentials on a
	// fresh session. Calling it here would conflict with the active QR session.
	return cred, nil
}
...
func qrPost(ctx context.Context, sess *Session, endpoint string, formData map[string]string) (*http.Response, error) {
	...
	req, err := newRequest(ctx, http.MethodPost, endpoint, body)
	if err != nil {
		return nil, err
	}
	req.Header.Set("User-Agent", sess.UserAgent)
	maps.Copy(req.Header, qrHeaders)
	return doRequest(sess, req)
}
...
func qrWaitingScan(ctx context.Context, sess *Session, ver, code string) (*QRScannedData, error) {
	for {
		if ctx.Err() != nil {
			return nil, ctx.Err()
		}
		...
		if body.ErrorCode != 0 {
			return nil, fmt.Errorf("zalo_personal: scan error code %d: %s", body.ErrorCode, body.ErrorMessage)
		}
...
func qrGetUserInfo(ctx context.Context, sess *Session) (*QRUserInfo, error) {
	req, err := newRequest(ctx, http.MethodGet, "https://jr.chat.zalo.me/jr/userinfo", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("User-Agent", sess.UserAgent)
...
// readJSON decodes a potentially gzip-encoded JSON response.
