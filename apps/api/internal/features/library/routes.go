...
func RegisterRoutes(rg *gin.RouterGroup, h *Handler, auth ...gin.HandlerFunc) {
	...
	g.DELETE("/versions/:vid/lessons", h.clearLessons)
	g.GET("/versions/:vid/exercise-groups", h.listExerciseGroups)
	g.POST("/versions/:vid/exercise-groups", h.createExerciseGroup)
	g.DELETE("/versions/:vid/exercise-groups/:gid", h.deleteExerciseGroup)
	g.GET("/lessons/:lid", h.getLesson)
	g.PUT("/lessons/:lid", h.updateLesson)
	g.DELETE("/lessons/:lid", h.deleteLesson)
	g.POST("/lessons/:lid/duplicate", h.duplicateLesson)
	...
	g.PATCH("/materials/:id/status", h.setMaterialStatus)
	...
	g.PATCH("/exercises/:id/status", h.setExerciseStatus)
}
