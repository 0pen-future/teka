...
func TestTemplateCreateWithLessonCountOverHTTP(t *testing.T) {
	r, d := newHTTPTest(t)
	reader, editor := mintToken(t, d.reader), mintToken(t, d.editor)
	...
	tpl := decode[TemplateResponse](t, env)
	if tpl.LessonCount != 2 {
		t.Fatalf("summary must show the seeded lessons, got %+v", tpl)
	}
	...
	if lessons := decode[[]LessonResponse](t, env); len(lessons) != 2 {
		t.Fatalf("lesson_count must seed 2 lessons, got %+v", lessons)
	}
}
