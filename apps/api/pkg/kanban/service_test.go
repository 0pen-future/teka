...
func TestMoveTaskSetsAndClearsCompletedAt(t *testing.T) {
	...
	moved, err := env.svc.MoveTask(context.Background(), tenant, plainActor(creator), task.ID, done.ID)
	require.NoError(t, err)
	require.NotNil(t, moved.CompletedAt)
	require.True(t, moved.CompletedAt.Equal(fixedNow))

	movedBack, err := env.svc.MoveTask(context.Background(), tenant, plainActor(creator), task.ID, todo.ID)
	require.NoError(t, err)
...
func TestMoveTaskAssigneeCanMoveButNotEdit(t *testing.T) {
	...
	_, err := env.svc.MoveTask(context.Background(), tenant, plainActor(assignee), task.ID, done.ID)
	require.NoError(t, err)
...
func TestMoveTaskStrangerForbidden(t *testing.T) {
	...
	_, err := env.svc.MoveTask(context.Background(), tenant, plainActor(newActor()), task.ID, done.ID)
	require.ErrorIs(t, err, ErrForbidden)
}
...
func TestMoveTaskTargetColumnNotFound(t *testing.T) {
	...
	_, err := env.svc.MoveTask(context.Background(), tenant, plainActor(creator), task.ID, ColumnID(uuid.New()))
	require.ErrorIs(t, err, ErrColumnNotFound)
}
