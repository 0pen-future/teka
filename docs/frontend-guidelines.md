...
path-prefix list exists separately from the entries' own `to` values because
a period-scoped link (e.g. "Chốt sổ") is `null` until the current period
resolves, yet its tab must still highlight while a matching route is open.
Missing either registration breaks only the mobile view, not desktop, so it
is easy to miss without deliberately checking both sets when adding an entry.
